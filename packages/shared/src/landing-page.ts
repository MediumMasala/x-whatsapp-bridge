 ...
 * Creates a minimal, mobile-first HTML page that works in X's in-app browser
 */

import type { LandingPageData } from './types';
import { escapeHtml, SECURITY_HEADERS } from './security';
 ...
 * - Primary CTA requires user tap (no auto-redirect)
 * - Fallback instructions for stubborn browsers
 * - Copy message functionality
 */
export function generateLandingPageHtml(data: LandingPageData): string {
  const { cid, phoneNumber, messageText, waHttpsUrl, waDeepLinkUrl } = data;
...
<body>
  ...
  <script>
    (function() {
      // Data passed from server (escaped)
      ...
      // CTA click handler - attempts deep link first, then HTTPS fallback
      ctaBtn.addEventListener('click', function(e) {
        ...
        // After 800ms, try the HTTPS fallback
        // This runs if deep link didn't navigate away
        setTimeout(function() {
